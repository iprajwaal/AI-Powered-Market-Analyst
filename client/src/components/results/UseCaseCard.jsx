    ...
    <Card className="mb-6">
      ...
      <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <h4 className="mt-4 text-sm font-medium text-gray-500">Business Value</h4>
          <p className="mt-1 text-gray-900">{useCase.business_value}</p>
          
          {useCase.prioritization_rationale && (
            <>
              <h4 className="mt-4 text-sm font-medium text-gray-500">Prioritization Rationale</h4>
              <p className="mt-1 text-gray-900">{useCase.prioritization_rationale}</p>
            </>
          )}
        </div>
        ...
        <div>
          ...
          {useCase.cross_functional_benefits && useCase.cross_functional_benefits.length > 0 && (
            <>
              <h4 className="mt-4 text-sm font-medium text-gray-500">Cross-Functional Benefits</h4>
              <div className="mt-1 space-y-2">
                {useCase.cross_functional_benefits.map((benefit, i) => (
                  <div key={i} className="flex items-start">
                    <span className="text-sm font-medium text-gray-500 w-32">{benefit.department}:</span>
                    <span className="text-sm text-gray-900">{benefit.benefit}</span>
                  </div>
                ))}
              </div>
            </>
      ...
      {resources && resources.length > 0 && (
        <div className="mt-6">
          <h4 className="text-sm font-medium text-gray-500">Implementation Resources</h4>
              ...
              <div key={i} className="flex items-start">
                <div className="flex-1">
                  ...
                  {resource.relevance_notes && (
                    <p className="mt-1 text-sm text-gray-600">{resource.relevance_notes}</p>
                  )}
                </div>
              </div>
            ))}
