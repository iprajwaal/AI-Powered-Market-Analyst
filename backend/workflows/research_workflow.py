...
def create_research_workflow(llm: LLMInterface, search_tools: SearchTools):
    ...
    async def enhance_research(state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if company_info:
            ...
            prompt += f"Products/Services: {', '.join(company_info.get('products', []))}\n"
            prompt += f"Competitors: {', '.join(company_info.get('competitors', []))}\n\n"
        ...
        if industry_info:
            ...
            prompt += f"Major Players: {', '.join(industry_info.get('major_players', []))}\n"
...
"""
    ...
    async def research_workflow(input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "industry_name" in state and state["industry_name"] or "company_info" in state:
