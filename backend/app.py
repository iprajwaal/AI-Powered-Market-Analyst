...
async def run_analysis(request_id: str, company_name: Optional[str], industry_name: Optional[str], num_use_cases: int):
...
async def analyze(request: AnalysisRequest, background_tasks: BackgroundTasks):
    ...
    if not request.company_name and not request.industry_name:
        raise HTTPException(status_code=400, detail="Either company_name or industry_name must be provided")
