...
class AnalysisTools:
            ...
            "properties": {
                "prioritized_use_cases": {
                    ...
                    "items": {
                        ...
                        "properties": {
                            "title": {"type": "string"},
                            "priority_score": {"type": "number", "minimum": 1, "maximum": 10},
    ...
    """
        ...
        try:
            ...
            priority_map = {pc["title"]: pc["priority_score"] for pc in prioritized_cases}
            rationale_map = {pc["title"]: pc["rationale"] for pc in prioritized_cases}
            
            for uc in use_cases:
                title = uc.get("title", "")
                uc["priority_score"] = priority_map.get(title, 5)  # Default to middle priority
                uc["prioritization_rationale"] = rationale_map.get(title, "")
            
            # Sort use cases by priority score (descending)
            sorted_use_cases = sorted(use_cases, key=lambda x: x.get("priority_score", 0), reverse=True)
