...
from config.settings import SERPAPI_KEY, SEARCH_CONFIG
...
class SearchTools:
    ...
    async def search_company_info(self, company_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if product_results:
                product_snippets = [result.get("snippet", "") for result in product_results[:3]]
            ...
            if competitor_results:
                company_info["competitors"] = [result.get("title", "").replace(" - Wikipedia", "") 
    ...
    async def search_industry_info(self, industry_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if trend_results:
                industry_info["trends"] = [result.get("snippet", "") for result in trend_results[:3]]
            
            # Search for industry challenges
            challenge_results = await self.search(f"{industry_name} industry challenges problems")
            if challenge_results:
                industry_info["challenges"] = [result.get("snippet", "") for result in challenge_results[:3]]
            
            # Search for major players
            player_results = await self.search(f"top companies in {industry_name} industry")
            if player_results:
                industry_info["major_players"] = [result.get("title", "").split(" - ")[0] 
                                               for result in player_results[:5]]
            
            # Search for AI applications in the industry
            ai_results = await self.search(f"artificial intelligence applications in {industry_name} industry")
            if ai_results:
                industry_info["ai_applications"] = [result.get("snippet", "") for result in ai_results[:3]]
